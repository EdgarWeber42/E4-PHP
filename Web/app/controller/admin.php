...
class admin extends Controller
{
    ...
    public function index()
    {
        if (isset($_SESSION['role'])) {
            if ($_SESSION['role'] == 'admin') {
                ...
                echo "<h3 class='text-center'>Bonjour Admin!</h3>";
    ...
    public function addCategory()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            //var_dump($_SESSION);
            if ($con->getNumRows() > 0) {
                $_SESSION['cart_msg'] = "Requete Ignorer! Cette catégorie existe déjà!";
                $_SESSION['cart_dialog'] = 0;
            } else {
                $_SESSION['cart_msg'] = "Catégorie Ajouter!";
                $_SESSION['cart_dialog'] = 0;
    ...
    public function editCategory()
    {
        ...
        if (isset($_POST['delete'])) {
            ...
            $new_cat = $con->secureInput($_POST['new_cat']);
            $cid = $con->secureInput($_POST['clist']);
            $sql = "DELETE FROM `AlgoBreizhdb`.`tbl_category` WHERE `tbl_category`.`category_id` = $cid ";
            $res = $con->doQuery($sql);
        ...
        public function editProduct()
        {
            if (isset($_POST['submit'])) {
                ...
                // ADD USER
                $sql = "INSERT INTO `tbl_products` SET `product_name` = '$name', 
                        `customer_address` = '$uaddress' , `customer_phone` = '$umobile' WHERE `tbl_customer`.`customer_id` = $id;";
                $res = $con->doQuery($sql);
                $_SESSION['cart_msg'] = "Produit ajoute !";
                $_SESSION['cart_dialog'] = 0;
            ...
            if (isset($_POST['submit'])) {
                ...
                // UPDATE USER
                $sql = "UPDATE `tbl_customer` SET `customer_name` = '$name', 
                        `customer_address` = '$uaddress' , `customer_phone` = '$umobile' WHERE `tbl_customer`.`customer_id` = $id;";
                $res = $con->doQuery($sql);
                $_SESSION['cart_msg'] = "Utilisateur modifier!";
                $_SESSION['cart_dialog'] = 0;
            ...
            if (isset($_POST['delete'])) {
                ...
                // DELETE USER
                $sql = "DELETE FROM `AlgoBreizhdb`.`tbl_customer` WHERE `tbl_customer`.`customer_id` = $id;";
                $res = $con->doQuery($sql);
                $_SESSION['cart_msg'] = "Utilisateur supprimer!";
                $_SESSION['cart_dialog'] = 0;
            }
            ...
            require APP . 'view/templates/admin_header.php';
            require APP . 'view/templates/admin_body.php';
            require APP . 'view/view.update_user.php';
            if (isset($_SESSION['cart_msg']) && isset($_SESSION['cart_dialog'])) {
                if ($_SESSION['cart_dialog'] == 0) {
                    $_SESSION['cart_dialog'] = 1;
                    Util::setNotification($_SESSION['cart_msg']);
    ...
    public function editUser()
    {
        if (isset($_POST['submit'])) {
            ...
            // ADD USER
            $sql = "INSERT INTO `tbl_customer` SET `customer_name` = '$name', 
                    `customer_address` = '$uaddress' , `customer_phone` = '$umobile' WHERE `tbl_customer`.`customer_id` = $id;";
            $res = $con->doQuery($sql);
            $_SESSION['cart_msg'] = "Utilisateur ajouter!";
            $_SESSION['cart_dialog'] = 0;
    ...
    public function updateView($id)
    {
        ...
        $pname = $rowu['product_name'];
        $pcat = $rowu['category_id'];
        $war = $rowu['product_warranty'];
        $stock = $rowu['stock_available'];
        $price = $rowu['price'];
        $des = $rowu['description'];
        $fet = $rowu['features'];
        $link = $rowu['product_image'];
        $cats = Products::getCategory();
        $scat = Products::getCategoryById($pcat);
        echo '<div class="row">
                    <form action="http://localhost/AlgoBreizh/web/admin/update" class="form-horizontal" method="post" enctype="multipart/form-data">
                        <div class="col-md-6">
                                <div class="form-inline">
                                    <label class="control-label" for="pname">Nom du produit</label>
                                    <input class="form-control" type="text" name="pname" id="pname"
                                           value="' . $pname . '" required>
                                </div>
                            <br>
                            <div class="form-inline">
                                <label class="control-label" for="pcat">Catégorie</label>
...
<div class="from-group" >
    ...
    public function update()
    {
        ...
        if (isset($_POST['update'])) {
            ...
            $war = $_POST['war'];
            $price = $_POST['price'];
            $dir = "/var/www/netbeans/AlgoBreizh/web/res/img/";
